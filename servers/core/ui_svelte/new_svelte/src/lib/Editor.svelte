<script lang="ts">
	...
	const handleInputs = async (event: KeyboardEvent) => {
		const e = event;
	...
	const constructSyntaxLines = async (node: Parser.SyntaxNode, lines: SyntaxLine[][]) => {
		if (node.isNamed()) {
...
<div id="editor" class="h-full w-full overflow-y-scroll bg-gray-700 pl-2 text-white">
